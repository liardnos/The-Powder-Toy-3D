...
void *lld_pop_node_start(lld_t *lld, long int pos)
{
...
void *lld_pop_node_end(lld_t *lld, long int pos)
{
    //if (pos >= lld->data) write(2, "db lld pop >= lld->data\n", 21);
    lld_t *lld_o = lld;
    lld->data = (void *)((u64)lld->data - 1);
    lld = lld->prev;
    for (int i = (int)lld_o->data; pos < i; lld = lld->prev, i--);
    if (lld->prev)
        lld->prev->next = lld->next;
    else
        lld_o->next = lld->next;
...
void *lld_pop_node(lld_t *lld, int poss)
{
    ...
    if ((long int)lld->data/2 >= (long int)pos){
        lld = (lld_t *)lld_pop_node_start(lld, pos);
    } else {
        lld = (lld_t*)lld_pop_node_end(lld, pos);
    }
    return (lld);
}
